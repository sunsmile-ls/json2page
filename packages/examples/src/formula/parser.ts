...
// 思路：
// 1. 先计算 token lexer  string -> token
// 2. 根据 运算符优先级递归调用，处理相应的数据  parser token -> ast
//    https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Operators/Operator_precedence
// 3. 计算好的 ast，最后通过计算得到 value evalutor   ast -> value
...
export function parse(input: string, options?: ParserOptions): ASTNode {
  ...
  // tokens 最后数据存储到 tokenChunk 前面  修改 token 变量
  function back() {
    tokenChunk.unshift(tokens.pop()!)
    token = tokens[tokens.length - 1]
  }

  // matchPunctuator 匹配操作数
  // 数组索引取反、字符串判等
  ...
  function skipWhiteSpaceChar() {
    while (token.type === TokenName[TokenEnum.Char] && /^\s+$/m.test(token.value)) {
      next()
    }
  }
  ...
  function postfixExpression(parseFunction: () => any = leftHandSideExpression): ASTNodeOrNull {
    let ast = parseFunction()
    if (!ast) {
      return null
    }
    ...
    while (matchPunctuator('[') || matchPunctuator('.')) {
      ...
      next()
      const right = assert(isDot ? identifier() || numberLiteral() || rawScript() : expression())
