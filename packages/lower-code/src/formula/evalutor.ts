...
export class Evaluator {
  ...
  constructor(
    ...
    this.contextStack.push((varname: string) => (varname === '&' ? context : context?.[varname]))
  ...
  evalute(ast: any) {
    if (ast && ast.type) {
      // "_x"或"-y" 去掉_ 和 - 并且后面的字符变为大写
      const name = (ast.type as string).replace(/(?:_|\-)(\w)/g, (_, l) => l.toUpperCase())
  ...
  filter(ast: { type: 'filter'; input: any; filters: Array<{ name: string; args: Array<any> }> }) {
      ...
      input = fn.apply(
        ...
        [input].concat(
          filter.args.map((item: any) => {
            if (item?.type === 'mixed') {
              return item.body
                .map((item: any) => (typeof item === 'string' ? item : this.evalute(item)))
                .join('')
            } else if (item.type) {
  ...
  nsVariable(ast: { namespace: string; body: any }) {
    if (ast.namespace === 'window') {
      this.contextStack.push((name: string) => (name === '&' ? window : (window as any)[name]))
      ...
      this.contextStack.push((name: string) => {
        const raw = ns === 'ss' ? sessionStorage.getItem(name) : localStorage.getItem(name)
  ...
  conditional(ast: { type: 'conditional'; test: any; consequent: any; alternate: any }) {
  ...
  funcCall(this: any, ast: { identifier: string; args: Array<any> }) {
    ...
    const fn =
      ...
      this[fnName] ||
      (this.filters.hasOwnProperty(ast.identifier) && this.filters[ast.identifier])
  ...
  callAnonymousFunction(
    ...
    const ctx: any = createObject(this.contextStack[this.contextStack.length - 1]('&') || {}, {})
    ...
    this.contextStack.push((varName: string) => (varName === '&' ? ctx : ctx[varName]))
   ...
   * @returns {Date} 日期对象
   */
  fnDATE(year: number, month: number, day: number, hour: number, minute: number, second: number) {
    if (month === undefined) {
   ...
   */
  fnDATETOSTR(date: Date | string | number, format: string = 'YYYY-MM-DD HH:mm:ss') {
   ...
   * @returns {string} 日期字符串
   */
  fnDATERANGESPLIT(daterange: string, key?: string, format?: string, delimiter = ',') {
    ...
    const dateArr = daterange
      .split(delimiter)
      .map((item) =>
        item && format ? moment(this.normalizeDate(item.trim())).format(format) : item.trim()
      )
   ...
   * @returns {boolean} 结果
   */
  fnISTYPE(target: any, type: 'string' | 'number' | 'array' | 'date' | 'plain-object' | 'nil') {
    switch (type) {
      case 'string':
