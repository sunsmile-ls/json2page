...
test('evalute:anonymous:function', () => {
  ...
  expect(evaluate('${ARRAYMAP(arr2, (item, index) => `a${item.a}${index}`)}', data)).toMatchObject([
    'a10',
    'a21',
    'a32',
  ])
  expect(
    evaluate('${ARRAYMAP(arr2, (item, index) => `a${item.a}${index}${outter}`)}', data)
  ).toMatchObject(['a104', 'a214', 'a324'])
  expect(
    evaluate('${ARRAYMAP(arr2, (item, index) => {x: item.a, index: index})}', data)
  ).toMatchObject([
    {
      x: 1,
      index: 0,
...
test('evalute:anonymous:function2', () => {
  ...
  expect(
    evaluate('${ARRAYMAP(ARRAYMAP(arr, item => item * 2), item => item + 2)}', data)
  ).toMatchObject([4, 6, 8])

  expect(evaluate('${ARRAYMAP(arr2, item => ARRAYMAP(item.y, i => i.z))}', data)).toMatchObject([
    [1, 1],
    [2, 2],
  ])
})
...
test('evalute:array:func', () => {
  ...
  expect(evaluate("${COMPACT([0, 1, false, 2, '', 3])}", data)).toMatchObject([1, 2, 3])
  ...
  expect(evaluate('${CONCAT(arr3, arr4)}', data)).toMatchObject([1, 2, 3, 2, 4, 6])

  expect(evaluate('${CONCAT(arr, arr4)}', data)).toMatchObject([2, 4, 6])

  expect(evaluate('${UNIQ(arr5)}', data)).toMatchObject(data.arr5)

  expect(evaluate('${UNIQ(arr5, "id")}', data)).toMatchObject([{ id: 1.1 }, { id: 2.2 }])

  expect(evaluate('${ARRAYFILTER(arr1, item => item)}', data)).toMatchObject([1, 2, 3])
  expect(evaluate('${ARRAYFILTER(arr1, item => item && item >=2)}', data)).toMatchObject([2, 3])

  expect(evaluate('${ARRAYFINDINDEX(arr3, item => item === 2)}', data)).toBe(1)

  expect(evaluate('${ARRAYFIND(arr5, item => item.name === 1.3)}', data)).toMatchObject({
    id: 1.1,
    name: 1.3,
  })

  expect(evaluate('${ARRAYSOME(arr5, item => item.name === 1.3)}', data)).toBe(true)

  expect(evaluate('${ARRAYEVERY(arr5, item => item.name === 1.3)}', data)).toBe(false)
  ...
  expect(evaluate('${DECODEJSON("{\\"name\\":\\"amis\\"}")}', data)).toMatchObject(
    JSON.parse('{"name":"amis"}')
  )
})
