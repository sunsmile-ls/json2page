...
TokenName[TokenEnum.EOF] = 'EOF' // 文档结束符
...
export function lexer(input: string, options?: LexerOptions) {
  ...
  // 结束符的 token
  // 条件 index >= input.length， index大于输入的值
  ...
  // 处理 非 js 的字符串
  // 条件：mainState 为 mainStates.START， 首次会进入
  ...
  // 处理 ${} 形式
  // 条件：非 Template 和 EXPRESSION 进入
  // ! 标记 mainState 为  SCRIPT
  function openScript() {
  ...
  function expression() {
    ...
    const token =
      literal() || identifier() || numberLiteral() || stringLiteral() || punctuator() || char()
  ...
  function punctuator() {
    const find = punctuatorList.find(
      (punctuator) => input.substring(index, index + punctuator.length) === punctuator
    )
    if (find) {
      return {
        type: TokenName[TokenEnum.Punctuator],
  ...
  // 处理 true | false | undefined 字面量类型
  // ! 获取最多 10 个字符（包含字母、数字下划线）进行判断
  function literal() {
    // {4,10} 匹配长度就足够判断  ("true").length <= targetLength <= ("undefined").length + 1
    // 获取最多 10 个字符（包含字母、数字下划线）
  ...
  // identifier 处理变量的情况
  // variableMode ? 处理 xxx : xxx.xxx || xxx[ expression ]
  function identifier() {
